...
class AdminSettings extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch(`https://api.questumd.com/api/persons?user_id=${decoded.identity.id}`).then(res => res.json()).then(data => {
      ...
      fetch(`https://api.questumd.com/api/tas?person_id=${person_id}`).then(res => res.json()).then(data => {
        var json = data[0]
        this.setState({
          office: json.office,
  ...
  onUpdate(e) {
    ...
    const requestOptionsPut = {
      ...
      // update fields
      fetch(`https://api.questumd.com/api/tas/${this.state.ta_id}`, requestOptionsPutQuestee).then(res => {
          if(res.ok){
              res.json().then(data => {
                  alert("Updated User Settings");
                  window.location.reload(false);
