...
const TeamInfo = (props) => {
    ...
    const saveChanges = async (e) => {
        ...
        for (var i = 0; i < teamData.questee_ids.length; i++) {
            let teamDeleteResponse = await fetch(`${apiLink}/team/${teamData.id}/questee/${teamData.questee_ids[i]}`, requestOptionsDelete);
        ...
        for (var i = 0; i < newTeamUids.length; i++) {
            let teamPostResponse = await fetch(`${apiLink}/team/${teamData.id}/questee/${newTeamUids[i]}`, requestOptionsPost);
    ...
    const deleteTeam = (e) => {
        e.preventDefault()
            ...
            //Deleting relations first
            //First getting project
            fetch(`${apiLink}/projects?team_id=${id}`).then(res => res.json()).then(project => {
                //Checking if project exists
                console.log(project)
                if (project[0]) {
                    alert("This team is still linked to a project. Please delete or update the project first")
                    ...
                    for (var i = 0; i < oldTeamUids.length; i++) {
                        fetch(`${apiLink}/questee/${oldTeamUids[i]}/team/${id}`, requestOptionsDelete).then(res => {
                        }).catch(console.log);
                    }
                    //finally deletinng team
                    fetch(`${apiLink}/team/${id}`, requestOptionsDelete).then(res => res.json()).then(data => {
