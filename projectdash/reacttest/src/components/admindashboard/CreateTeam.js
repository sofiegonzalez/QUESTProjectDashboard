...
const CreateTeam = (props) => {
  ...
  const [semesterYear, setSemesterYear] = useState("2019");
    ...
    if (isValidListOfMembers(newTeamMembers)) {
      ...
      fetch(`${apiLink}/teams`, requestOptionsTeam).then(res => res.json()).then(team => {
        ...
        for (var i = 0; i < newTeamUids.length; i++) {
          fetch(`${apiLink}/team/${team[0].id}/questee/${newTeamUids[i]}`, requestOptionsPost).then(res => {
        ...
        for (var i = 0; i < newTeamUids.length; i++) {
          ...
          fetch(`${apiLink}/questee/${newTeamUids[i]}`, requestOptionsGet).then(res => res.json()).then(oldTeam => {
            ...
            if (!oldTeam[0].team_ids.includes(team[0].id)) {
              //Add new team id to questees list of ids
                fetch(`${apiLink}/questee/${newTeamUids[i]}/team/${team[0].id}`, requestOptionsPost).then(res => {
  ...
  const verifyMember = (index) => {
    ...
    if (typeof enteredUid == 'string' && enteredUid.length != 0) {
      ...
      fetch(`${apiLink}/questees?uid=${enteredUid}`, requestOptionsGet).then(res => res.json()).then(questeeData => {
        console.log(questeeData)
        if (questeeData.length == 0) {
          alert("Entered UID is not registered in our system. Please try another.");
          return;
        }
        fetch(`${apiLink}/person/${questeeData[0].person_id}`, requestOptionsGet).then(res => res.json()).then(personData => {
